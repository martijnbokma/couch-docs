...
Related editable regions can be grouped together by setting the `group` parameter of each region to the name of an another editable region of type [`group`](./editable/group.html):
...
`hidden` parameter can be set to `1` to suppress the output of an editable region that has been defined outside the [`template`](./template.html) tag.
