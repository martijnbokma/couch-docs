...
keywords: [breadcrumbs, CouchCMS, Couch, tags, PHP]
...
If this parameter is set, remember to check that the current view is either _folder-view_ or _page-view_ before invoking this tag, because for any other views the folder hierarchy is irrelevant and **breadcrumbs** tag rightly does not output any folder names, however the _include_template_ parameter will cause the template's name to appear which might not be desirable.
