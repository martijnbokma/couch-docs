...
keywords: [embed, CouchCMS, Couch, tags, PHP, template, include, snippets]
...
:::tip[Update]
From version 1.1.1, the base path relative to which Couch searches for embedded files can be changed from the default 'couch/snippets' folder to any folder **relative to your main site**. This can be done by setting the `12b. K_SNIPPETS_DIR` configuration item within your _config.php_ file.
...
:::caution
Make sure that the type of the editable region is _textarea_ and that its _no_xss_check_ parameter is set to '1' (this will prevent Couch from mangling up the < and > tags the way it usually does to stop XSS attacks).
:::
