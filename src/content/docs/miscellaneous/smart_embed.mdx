...
description: "Learn how to use the smart_embed tag to automatically structure and organize your CouchCMS templates"
keywords:
    - smart embed
    - templates
    - views
...
:::note
The 'smartness' of the `smart_embed` tag lies in its capability to look into the snippets folder and then **automatically choose the snippet best matching the current view and then embed it**.
...
[**Nested pages**](../../concepts/nested-pages) behave more or less like folders.
For example, if we have a nested-page named `harpoon` that is nested below another pages named `chinese`, this is the sequence of search while visiting `harpoon`:
...
The real power of `smart_embed` tag comes to fore when we combine it whith the practice of 'chunking' up of templates.
...
The `smart_embed` tag accepts a second (optional) parameter using which we can specify the folder where the tags searches for the matching snippets.
...
Run this snippet and you'll find that the `smart_embed` tag outputs the 'default.html' snippets for each chunk area.
...
What we are concerned with is that it uses _exactly_ the same `smart_embed` statements.
...
In certain situations, we can also combine the `smart_embed` with the regular [**embed**](../../tags-reference/embed.html) tag.
...
:::tip
I find it tedious to flip the debug parameter if there are multiple `smart_embed` statements in my template.
