...
Thus if, for example, comments are to be turned on for pages cloned out of `blog.php`, the following change needs to be done to the [__*template*__](../../tags-reference/template.html) tag (usually placed in the HTML head tag) within blog.php:
...
:::tip
The above snippet can be saved into a file named, for example, `comment-form.inc` within the snippets folder and then be used in any template by using the [__*embed*__](../../tags-reference/embed.html) tag:
...
On successful submission of the form (*k_success*), we simply invoke the [__*process_comment*__](../../tags-reference/process_comment.html) tag.
