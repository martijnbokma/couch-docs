---
title: Cloned Pages
description: "Learn how to create multiple pages from a single template in CouchCMS. Master clonable templates, page views, and list views for dynamic content management."
keywords:
    - cloned pages
    - clonable templates
    - page views
...
To make these pages updatable by the users, it is sufficient to mark out the updatable HTML areas in the templates using the [**editable**](../../tags-reference/editable.html) tags and Couch will make them editable.

Some other templates, e.g. `blog.php` or `property.php`, cannot possibly represent one single page.
...
A template can be marked as being clonable by using the [**template**](../../tags-reference/template/) tag somewhere within it and setting its _clonable_ parameter to '1'.
Place the following snippet somewhere at the beginning of the template (within the HEAD tag will do just fine):
...
Going back to the dashboard, create a new page from `blog.php` and we'll be shown the same three editable regions (along with the _Name_ and _Title_ fields).
...
Thus a non-clonable template like `about-us.php`, when visited like this:
...
\- `blog.php` is not representing any page (there is no '?=p' present). It is, well, simply itself.
...
\- `blog.php` is clueless about which cloned page's data it should display and since it has no data of its own, the updatable regions come up empty.
...
useless? Not at all. By default Couch displays nothing here but it is totally upto you as to what you wish to show on this page.
...
As a temporary measure, you can turn off Pretty URLs in `_config.php` till you are done with the testing.
:::
