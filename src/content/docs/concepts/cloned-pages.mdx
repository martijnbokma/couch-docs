---
title: Cloned Pages
description: Learn how to create multiple pages from a single template using CouchCMS's cloning feature. Understand the concept of clonable templates, page views, and list views.
keywords: [cloned pages, clonable templates, page views, list views, template cloning, multiple pages, dynamic content, content management, CMS templates]
category: concept
---
...
A template can be marked as being clonable by using the [**template**](../../tags-reference/template.html) tag somewhere within it and setting its _clonable_ parameter to '1'.
...
:::note
It becomes even more distinct when we use pretty urls (see [**Pretty URLs**](../pretty-urls.html)).
...
:::danger[Important]
If you have turned on the [**Pretty URLs**](../pretty-urls.html) feature, every time you declare a template to be clonable, make sure to recreate your `.htaccess` file else the cloned pages will NOT be accessible.
