...
title: Folders
category: concept
description: Learn how to organize your CouchCMS pages using virtual folders and create SEO-friendly URLs
keywords: [folders, virtual folders, folder hierarchy, SEO URLs, CouchCMS folders]
---
...
For creating folders in Couch we use the [__*folder*__](../../tags-reference/folder/) tag. Hierarchy can be created by nesting one [__*folder*__](../../tags-reference/folder/) tag (the child) within another (the parent).
...
:::note
These tags can be placed anywhere as they have no output of their own. However it is a good practice to define all the folders of a template (along with all the editable regions) within the [__*template*__](../../tags-reference/template/) tag placed somewhere at the top of the template.
:::
...
The [__*folders*__](../../tags-reference/folders/) tag is used to get a list of all the folders in a template.

:::tip
Notice the 's' suffixed to the tag. The [__*folder*__](../../tags-reference/folder/) tag creates folders while the [__*folders*__](../../tags-reference/folders/) tag lists folders.
:::
...
Since the [__*folders*__](../../tags-reference/folders.html) tag was provided with no information about which template's folders it should enumerate, it by default did so for the template it was called from, i.e. `news.php`.
...
For a complete list of all the variables that get set by this tag please see the [reference](../../tags-reference/folders/#parameters) or use Couch tags [__*dump*__](../../tags-reference/dump/) or [__*dump_all*__](../../tags-reference/dump_all/) within the [__*folders*__](../../tags-reference/folders/) tag.
:::
...
You can control what gets enumerated by the [__*folders*__](../../tags-reference/folders/) tag by setting its various [**parameters**](../../tags-reference/folders/#parameters).
...
[__*Folders*__](../../tags-reference/folders/) tag makes it easy for you to create lists like above. Please notice in the HTML code above how a &lt;UL&gt; marks the beginning of a new nested level and how a &lt;LI&gt; marks the beginning of a new element (folder in our case).

Setting the *extended_info* parameter to 1 will make the [__*folders*__](../../tags-reference/folders/) tag provide extra information about every change in level as well as in current element. Our snippet simply has to respond to the reported changes.
...
With *extended_info* set, the [__*folders*__](../../tags-reference/folders/) tag walks through the folder tree and sends back information about its current position in the hierarchy (by setting different variables). Simply outputting the HTML tag relevant to the current state will create a perfect list.

:::tip
Incidentally, if a simple list is all that is needed there is a much simpler option than the one above - the [__*listfolders*__](../../tags-reference/listfolders/) tag:
...
[__*Listfolders*__](../../tags-reference/listfolders/) tag accepts the same parameters as [__*folders*__](../../tags-reference/folders/) and is a quick way to get a generic list. The [__*folders*__](../../tags-reference/folders/) tag, however, gives you absolute control over what needs to be output.
:::

:::note
[__*Listfolders*__](../../tags-reference/listfolders/) takes one additional parameter - *show_count*. Setting this to 1 will make it display the number of pages contained within the folders after the folder name.
:::
...
Rest of the snippet is a modified form of the standard [__*folders*__](../../tags-reference/folders/) tag code using *extended_info*.

When *k_element_start* variable is found set, we output the enumerated folder's name as usual.
The only thing new is that we also find out if the folder (in *k_folder_name* variable) is one of the ancestors of the 'current_folder' we saved above by using the [__*is_ancestor*__](../../tags-reference/is_ancestor.html) tag.
...
The [__*breadcrumbs*__](../../tags-reference/breadcrumbs/) tag can be used to quickly create a breadcrumb:
...
Using [__*parentfolders*__](../../tags-reference/parentfolders/) gives you greater control on the HTML that is generated for the breadcrumb.
Notice how in the snippet above we first check for the view we are in and then find the right folder name (_folder-view_ sets the *k_folder_name* variable to the name of the folder being listed while _page-view_ sets *k_page_foldername* variable to the folder [if any] the current page resides in **[see [Views](../views/)]**). Finally once a folder is found, it is passed on to [__*parentfolders*__](../../tags-reference/parentfolders/) tag to get a list of its ancestors.
