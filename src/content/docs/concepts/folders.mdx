---
title: Folders
description: "Learn how to organize content using folders in CouchCMS."
keywords:
    - folders
...
By judiciously creating a hierarchy of folders, almost any URL structure can be mimicked in Couch.
...
<FileTree>
    - News/ (`news.php`) (root folder) - World&nbsp;News/ (sub-folder) - North&nbsp;American/ (sub-sub-folder) -
    United&nbsp;States&nbsp;News/ (sub-sub-sub-folder) - Ohio&nbsp;News/ (sub-sub-sub-sub-folder) - Nevada&nbsp;News/
    (sub-sub-sub-sub-folder) - Asian&nbsp;News/ (sub-sub-folder) - China&nbsp;News/ (sub-sub-sub-folder) -
    Japan&nbsp;News/ (sub-sub-sub-folder) - Sports&nbsp;News/ (sub-folder) - Music&nbsp;News/ (sub-folder) -
    Entertainment&nbsp;News/ (sub-folder)
</FileTree>

For creating folders in Couch we use the [**_folder_**](../../tags-reference/folder/) tag. Hierarchy can be created by nesting one [**_folder_**](../../tags-reference/folder/) tag (the child) within another (the parent).
To create the folder structure mentioned above, place the following snippet anywhere within the `news.php` template, and visit `news.php` in your browser while being logged-on as super-admin:
...
:::note
These tags can be placed anywhere as they have no output of their own. However it is a good practice to define all the folders of a template (along with all the editable regions) within the [**_template_**](../../tags-reference/template/) tag placed somewhere at the top of the template.
:::

:::tip[Update]
From version 1.1 Couch supports creation of folders from within the admin panel. Please see [**Dynamic Folders**](../../miscellaneous/dynamic-folders.html) for details.
:::
...
Now if a cloned page of `news.php` is edited in the admin panel, clicking on the 'Advanced settings' buttom at the top will reveal a dropdown list that will show these folders.
...
You can see how the page's url reflects with full fidelity its folder hierarchy.
...
One solution is to place a dummy page at the original URL and make it send back a 'HTTP 301 Permanently Moved' message to anyone accessing it and also informing him about the page's new location.
Another solution is to add an entry for it in `.htaccess` file.
All in all, the solutions are pretty messy and time consuming.

Couch handles this situation automatically behind the scenes. You can freely change the location of your pages. Anytime a page is accessed via a URL that is no longer valid, Couch sends back the visitor to the new location by using the 'HTTP 301 Permanently Moved' message.
...
The [**_folders_**](../../tags-reference/folders/) tag is used to get a list of all the folders in a template.

:::tip
Notice the 's' suffixed to the tag. The [**_folder_**](../../tags-reference/folder/) tag creates folders while the [**_folders_**](../../tags-reference/folders/) tag lists folders.
:::
...
Since the [**_folders_**](../../tags-reference/folders.html) tag was provided with no information about which template's folders it should enumerate, it by default did so for the template it was called from, i.e. `news.php`.
...
:::note
`k_folder_title` is only one of the variables made available by this tag to provide information about the folder being enumerated.
...
See: [**Listing Pages**](../listing-pages/).

For a complete list of all the variables that get set by this tag please see the [reference](../../tags-reference/folders/#parameters) or use Couch tags [**_dump_**](../../tags-reference/dump/) or [**_dump_all_**](../../tags-reference/dump_all/) within the [**_folders_**](../../tags-reference/folders/) tag.
:::
...
You can control what gets enumerated by the [**_folders_**](../../tags-reference/folders/) tag by setting its various [**parameters**](../../tags-reference/folders/#parameters).
...
[**_Folders_**](../../tags-reference/folders/) tag makes it easy for you to create lists like above. Please notice in the HTML code above how a `<ul>` marks the beginning of a new nested level and how a `<li>` marks the beginning of a new element (folder in our case).

Setting the _extended_info_ parameter to 1 will make the [**_folders_**](../../tags-reference/folders/) tag provide extra information about every change in level as well as in current element. Our snippet simply has to respond to the reported changes.
...
With _extended_info_ set, the [**_folders_**](../../tags-reference/folders/) tag walks through the folder tree and sends back information about its current position in the hierarchy (by setting different variables). Simply outputting the HTML tag relevant to the current state will create a perfect list.

:::tip
Incidentally, if a simple list is all that is needed there is a much simpler option than the one above - the [**_listfolders_**](../../tags-reference/listfolders/) tag:
...
[**_Listfolders_**](../../tags-reference/listfolders/) tag accepts the same parameters as [**_folders_**](../../tags-reference/folders/) and is a quick way to get a generic list. The [**_folders_**](../../tags-reference/folders/) tag, however, gives you absolute control over what needs to be output.
:::

:::note
[**_Listfolders_**](../../tags-reference/listfolders/) takes one additional parameter - _show_count_. Setting this to 1 will make it display the number of pages contained within the folders after the folder name.
:::
...
Notice how we first set the variable `current_folder` to the folder of the current page or the folder being viewed (variable `k_folder_name` gets set only in _folder-view_ while `k_page_foldername` gets set only in _page-view_ if the page resides in a folder).
Rest of the snippet is a modified form of the standard [**_folders_**](../../tags-reference/folders/) tag code using `extended_info`.

When `k_element_start` variable is found set, we output the enumerated folder's name as usual.
The only thing new is that we also find out if the folder (in `k_folder_name` variable) is one of the ancestors of the 'current*folder' we saved above by using the [\*\*\_is_ancestor*\*\*](../../tags-reference/is_ancestor.html) tag.
If it is, we simply set a class of 'selected' to the `<li>` and `<span>` elements being output.

To list only the ancestors of a folder in the hierarchy, [**_parentfolders_**](../../tags-reference/parentfolders.html) tag can be used. It lists all the parents of a folder in sequence.
...
The [**_breadcrumbs_**](../../tags-reference/breadcrumbs/) tag can be used to quickly create a breadcrumb:
...
The _include_template_ parameter adds the current template ('News' in this case) as the first element.

The same output can be obtained by using the [**_parentfolders_**](../../tags-reference/parentfolders.html) tag mentioned above:
...
Using [**_parentfolders_**](../../tags-reference/parentfolders/) gives you greater control on the HTML that is generated for the breadcrumb.
Notice how in the snippet above we first check for the view we are in and then find the right folder name (_folder-view_ sets the `k_folder_name` variable to the name of the folder being listed while _page-view_ sets `k_page_foldername` variable to the folder [if any] the current page resides in **[see [Views](../views/)]**). Finally once a folder is found, it is passed on to [**_parentfolders_**](../../tags-reference/parentfolders/) tag to get a list of its ancestors.
