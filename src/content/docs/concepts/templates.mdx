...
To start working with Couch, you need to have a regular HTML coded site in hand. As an example, your site could consist of several HTML files e.g. `index.html`, `services.html`, `about-us.html`, `contact-us.html` etc. along with the associated media, CSS and JavaScript files.
**Each of the above mentioned HTML files, for Couch, is a template.**

Couch works by retrofitting into these templates. Since the templates used by Couch are regular HTML files, any of your existing static HTML sites too can be easily ported to Couch with minimal effort.

<Card icon="star" title="Tutorial">

Please see the following tutorial, where we use Couch to build a full fledged site, for a step-by-step illustrated guide on how to install Couch - [**Building a real-world site**](../../tutorials/portfolio-site/01-building-real-world-site).

</Card>

## Installation

Follow the steps in [**Installation**](../../installation/) to install Couch.
...
Once we have Couch hooked into the template, we can place Couch [**Tags**](../tags/) in the template to make Couch work its magic.
