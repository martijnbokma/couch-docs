...
keywords: [paypal, payments, digital products, IPN, e-commerce]
category: concept
sidebar:
    order: 18
...
First we need to inform Couch about some of our PayPal details. Open `config.php` and set the following three values to that of yours:
...
Setup a test environment (sandbox) at https://developer.paypal.com/. Generate and use the fictitious accounts for testing (make sure to set `K_PAYPAL_USE_SANDBOX` to 1 and `K_PAYPAL_EMAIL` to the fictitious seller's account, in `config.php` mentioned above).
:::
...
For our example, let us say we use a template named `product.php` for this purpose.
Make this template clonable and define in it all [**editable regions**](../editable-regions/) that are relevant to your products (e.g. ISBN number, Author, Link to the downloadable file etc.)

Two fields are mandatory and need your special attention:

<Steps>
1. **Item's name** - every saleable item needs to have a name. However, you don't have to create a separate editable region for this because Couch expects the _Title_ field (present by default in all clonable pages and accessible via _k_page_title_ variable) to be the item's name. Hence always input the item's name as the title of the page representing the item.
...
2. **Item's price** - this is an important one.
    ...
    The important point is that - Couch requires this editable region to be named _pp_price_.
</Steps>
...
The image used as the button can be changed by setting the _image_ parameter of the tag (See [**Tags Reference - PayPal Button**](../../tags-reference/paypal_button/)).
...
[__*paypal_processor*__](../../tags-reference/paypal_processor/) is the tag that will handle IPNs for you.
...
Try creating a file named `log.txt` (or whatever you have set using _logfile_ parameter) manually, make it writable and try once again.
:::
...
Once we are sure that the IPN is being handled properly, we can now go ahead and make [__*paypal_processor*__](../../tags-reference/paypal_processor/) do whatever is supposed to be done on successfully getting paid for a product. We'll assume we are to send the buyer an email with a link to the item he bought.

Once [__*paypal_processor*__](../../tags-reference/paypal_processor/) verifies that the IPN is indeed from PayPal, it sets the following variables for use in your script -
...
In the snippet above, we are sending an email to the buyer on a successful transaction. Note how the variables set by [__*paypal_processor*__](../../tags-reference/paypal_processor/) are being used in the [__*send_mail*__](../../tags-reference/send_mail/) tag.
The email contains a link to the downloadable item (the variable *downloadable_file* is an editable region of type file within the template and contains the item's link).
The [__*cloak_url*__](../../tags-reference/cloak_url/) tag is used to conceal the real path of the item.
