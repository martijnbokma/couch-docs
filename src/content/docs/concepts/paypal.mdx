...
The image used as the button can be changed by setting the `image` parameter of the tag (See [**Tags Reference - PayPal Button**](../../tags-reference/paypal_button/)).

:::danger[Remember]
If the **paypal_button** tag fails to find a variable named _pp_price_, that we mentioned in the previous section, it will not display any button at all.
:::
...
[**_paypal_processor_**](../../tags-reference/paypal_processor/) is the tag that will handle IPNs for you.
...
Try creating a file named `log.txt` (or whatever you have set using `logfile` parameter) manually, make it writable and try once again.
:::
...
Once we are sure that the IPN is being handled properly, we can now go ahead and make [**_paypal_processor_**](../../tags-reference/paypal_processor/) do whatever is supposed to be done on successfully getting paid for a product. We'll assume we are to send the buyer an email with a link to the item he bought.

Once [**_paypal_processor_**](../../tags-reference/paypal_processor/) verifies that the IPN is indeed from PayPal, it sets the following variables for use in your script:
...
In the snippet above, we are sending an email to the buyer on a successful transaction. Note how the variables set by [**_paypal_processor_**](../../tags-reference/paypal_processor/) are being used in the [**_send_mail_**](../../tags-reference/send_mail/) tag.
...
The [**_cloak_url_**](../../tags-reference/cloak_url/) tag is used to conceal the real path of the item.
