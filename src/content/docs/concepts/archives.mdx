...
[**_Archives_**](../../tags-reference/archives/) tag is used to find out all the past time periods (years, months and days) during which atleast one page has been published.
...
:::note
The _k_archive_date_ variable is set in a machine readable format.
The [**_date_**](../../tags-reference/date/) tag converts it into a displayable format.
:::
...
The [**_archives_**](../../tags-reference/archives.html) tag is only the first step in arranging your pages in archives as it merely delineates the archive periods for us.
To list the pages belonging to any of these periods, we'll have to use the [**_pages_**](../../tags-reference/pages.html) tag which we have already discussed.

If you recall the [**_pages_**](../../tags-reference/pages.html) tag, pages belonging to a particular time period can be easily fetched by setting the _start_on_ and _stop_before_ parameters.
...
We can use the _k_archive_date_ and _k_next_archive_date_ variables set at each iteration of the archives tag to do so.
...
The _k_archive_link_ variable set by the archives tag points to this view for each time period.
...
By pointing your browser to these urls, you'll be accessing your `blog.php` template in _archive-view_ (see: [**Views**](../views/)).
In this view, apart from other variables that you normally find set, Couch sets the _k_archive_date_ and _k_next_archive_date_ variables (see: [**Variables available in Views**](../variables-in-views/)).
It shouldn't be difficult to set up the [**_pages_**](../../tags-reference/pages/) tag to list pages belonging the archive period by using these variables (see: [**Listing Pages**](../listing-pages/)).
