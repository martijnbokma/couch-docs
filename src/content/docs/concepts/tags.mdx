...
In Couch, naming the parameters is entirely optional but then you'll have to make sure that you pass the parameters in the exact sequence the tag expects them in. Thus, for example, [**repeat**](../../tags-reference/repeat/) tag can take two parameters _count_ and _startcount_ where _startcount_ is expected to be the second parameter. If we explicitly name the parameters, e.g.:
