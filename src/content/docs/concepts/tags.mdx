...
Similar to the XML tags, a Couch tag is either *self-closing* or consists of a *start-tag/end-tag* pair.
...
Every Couch tag begins with `<cms:` followed by the tag's name (`show` and `repeat` in the examples above).
