...
keywords: [RSS, feeds, syndication, XML, content distribution]
category: concept
---
...
The following snippet using Couch's [**concat**](../../tags-reference/concat.html) tag will output exactly the same statement but because the `<?` characters are now split up, PHP will have no problems:
...
The second problem can be rectified by using Couch's [**content_type**](../../tags-reference/content_type.html) tag. This tag instructs the web server to output the desired mime-type. We'll use it to output `text/xml`.
...
:::danger[Important]
Do not split the [**content_type**](../../tags-reference/content_type.html) statement and the [**concat**](../../tags-reference/concat.html) statement into separate lines. This will cause an empty line to be output before the xml declaration and will invalidate the feed.
:::

We can now fetch the news items from the `news.php` template by using [**pages**](../../tags-reference/pages.html) tag and add it to our feed to make it dynamic:
