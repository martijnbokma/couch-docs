...
If they are logged-in, the `k_user_id`, `k_user_name`, `k_user_title`, `k_user_email` and `k_user_access_level` can be used to get details about them.
...
Note how we use the `k_login_link` and `k_logout_link` to display the links that the user can click to either login or logout.
