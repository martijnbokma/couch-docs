...
URLs of clonable templates that have the '`?p=n`' appended are considered by Couch to be the '`page-views`' (i.e. displaying cloned pages) while the URLs without this information are considered as being the '`list-views`' \[see [Views](../../../../concepts/views.html) in documentation\].
...
:::note
You might recall from the documentation that apart from the variables set up by Couch (the system variables that are all prefixed by a 'k\_'), we can define our own variables. \[see [Variables](../../../../concepts/variables.html)\]
:::
