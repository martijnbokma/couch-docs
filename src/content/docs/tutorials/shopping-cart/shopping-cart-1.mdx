...
keywords: [shopping cart, CouchCMS, e-commerce, PayPal, products, variants, AJAX, tutorial]
---
...
+ **shipping_address**
    ...
    + If set to '2', means 'address required but will be supplied by us'. In this case the onus of capturing the buyer's address falls on us. This tag expects the elements of the address to be saved in session as variables of the following names: 'contact_email', 'shipping_first_name', 'shipping_last_name', 'shipping_address1', 'shipping_address2', 'shipping_city', 'shipping_state_code', 'shipping_country_code', 'shipping_zip'.
