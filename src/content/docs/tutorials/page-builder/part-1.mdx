...
:::note
If you are familiar with Couch, you are likely to notice that the existing ['Mosaic'](https://www.couchcms.com/forum/viewtopic.php?f=5&t=11105) feaure holds out a similar premise.
...
By default Couch expects all snippets to be within `couch/snippets` folder (this location can be changed from `couch/config.php` but I am assuming we are using the default) so I created a folder named 'pb' within it (again, an arbitrary name - pagebuilder shortened to pb)
and then created separate folders for each block types.
